...
- AI calls `aifp_run(is_new_session=true)` on first interaction of a session
- Returns full bundle: status, settings, directives, supportive context, guidance

**Work Checkpoints** (Organic):
- AI calls `aifp_run(is_new_session=false)` at work milestones (file finalization, task/milestone completion)
...
`aifp_run` does NOT execute tasks directly. It returns data based on mode:
...
2. Calls `aifp_status()` for fresh context (not aifp_run — status is the right tool here)
3. `aifp_status` returns:
...
1. **Always call on session start** - First interaction uses `aifp_run(is_new_session=true)`
