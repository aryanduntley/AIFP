"""
AIFP MCP Server - Server Core (Zero-Dependency)
...
SERVER_VERSION: Final[str] = "1.23.0"
...
def build_tool_annotations(tool_name: str) -> Dict[str, Any]:
    ...
    Classification:
      - Read-only: get_*, search_*, query_*, find_*, list_*, detect_*,
        plus special cases (aifp_run, *_has_changed, *_check_constraints)
      - Destructive: delete_*, execute_merge, aifp_end
      - Write (non-destructive): everything else
      All tools: openWorldHint=false (AIFP is entirely local).
    """
...
def _effect_load_and_cache_tools() -> None:
    ...
    try:
        ...
        for row in cursor.fetchall():
            name = row["name"]
            description = row["purpose"] or f"AIFP tool: {name}"
