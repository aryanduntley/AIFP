...
name: aimfp-mode
description: >
  AIMFP (AI Functional Procedural) behavioral framework. Activates when the AIMFP
  MCP server is connected. Enforces pure functional programming (no OOP, no
  mutations), directive-driven workflows, and database-driven project management
  across 4 SQLite databases (aimfp_core.db, project.db, user_preferences.db,
  user_directives.db). On every interaction: call aimfp_run() first, check project
  state, act proactively. Track all code in project.db via reserve-write-finalize.
  Use directives for project lifecycle: init, discovery, progression, completion.
user-invocable: false
---

# AIMFP MODE ACTIVE - AI FUNCTIONAL PROCEDURAL PROGRAMMING

## CRITICAL: PROACTIVE BEHAVIOR REQUIRED

AIMFP is NOT optional. When this MCP server is active, you MUST act proactively:

**On EVERY user interaction:**
1. **FIRST ACTION**: Call aimfp_run() BEFORE any other processing
   - Called on EVERY interaction — this is your reminder system
   - Do this immediately when user sends ANY message
   - Don't analyze, don't plan — just call aimfp_run first
   ...
   **Parameter usage:**
   ...
   - **aimfp_status()**: Call when AI detects its own context is stale or compressed and needs a fresh database pull. Returns project state + supportive context. If a user asks "where are we" or "status", answer from what you already know — only call aimfp_status() if your context is actually stale.

2. **Check project state** (from bundle or cached context):
   - .aimfp-project/ missing → check for .git/.aimfp/ backup → offer restore or aimfp_init
   - .aimfp-project/ exists → read ProjectBlueprint.md, identify next action from status
...
4. DO NOT sit idle waiting for commands. Use project state and directive flows to drive action.
...
1. **aimfp_init**: Creates databases, templates, base files
...
5. **aimfp_end**: Graceful session close (audit, verify DB, stop watchdog)
...
1. **Pure Functions Only** — Same inputs → same outputs. No side effects, no hidden state, explicit parameters.
   - Read-only global constants encouraged (Final, const). Mutable globals → use state database instead.
...
**Non-FP Projects**: If existing codebase is OOP-based → STOP, inform user AIMFP is FP-only.
...
**Key Directives**:
- **aimfp_run / aimfp_status**: Entry points (every interaction)
- **aimfp_init / project_discovery**: Setup and planning
...
- **aimfp_run(is_new_session)** — Entry point, bundles startup data
- **aimfp_status(project_root, type)** — Comprehensive project state
- **aimfp_init(project_root)** — One-time project initialization
- **aimfp_end(project_root)** — Session termination audit
...
1. **Status-First**: "continue"/"status"/"resume" → answer from existing context. Call aimfp_status() only if context is stale.
...
5. **Evolution Notes**: Any change to ProjectBlueprint.md, completion paths, themes, flows, or milestones MUST be accompanied by a notes table entry (note_type='evolution', source='ai') describing what changed and why. Use add_note helper. This is non-negotiable — the database is the project's memory.
6. **Recovery**: Stale/compressed context → aimfp_status() for fresh DB pull + supportive context. Need reference only → get_supportive_context(). Log decisions → project_notes_log.
   **Staleness check**: If you cannot recall the current milestone name and active task, your context is stale — call aimfp_status().
7. **Session End**: "done"/"wrap up" → aimfp_end for audit
...
Every interaction: aimfp_run → check state → present context → execute with FP baseline → loop to status or aimfp_end.
